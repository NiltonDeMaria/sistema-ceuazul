...
import {
  ...
  Th,
  Td,
  TableContainer, Box, Center, Flex,
...
function Read() {
  ...
  useEffect(() => {
    axios.get(`https://63c5cfbdf80fabd877f05d4d.mockapi.io/projetom3`)
      .then((response) => {
  ...
  const setData = (data) => {
    let { id, nomeResp, naturalResp, email, celular, telefone, endereco, cidade, nomeCrian, dataNasc, naturalCrian, tipoSang, sexo } = data;
  ...
  const onDelete = (id) => {
    axios.delete(`https://63c5cfbdf80fabd877f05d4d.mockapi.io/projetom3/${id}`)
      .then(() => {
        getData();
      })
  }
